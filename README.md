...
- write the output onto a file called `sol.tex` with `>`, if it doesn't exist, it will be created. `.tex` is the file format for `LaTeX`. Having `LaTeX` installed will be useful for these patterns. I will link how to download later.
- Compile the `LaTeX` document with `pdflatex`, and `sol.pdf` is the final product.
...
- (1) My output was always including the solutions written as well!
